...
def rep_list_role_permissions(state, session_file, role):
    ...
    if response.status_code == 200:
        ...
        print("\n+" + ("-"*bar_size) + f" {role} Permissions " + ("-"*bar_size) + "+\n")
...
def rep_list_permission_roles(state, session_file, permission):
    ...
    if response.status_code == 200:
        ...
        if permission_roles["type"] == "Organization roles":
            terminal_width = shutil.get_terminal_size().columns
            bar_size = int((terminal_width - len(f"Organization roles with {permission} permission") - 4) / 2)
            print("\n+" + ("-"*bar_size) + f" Organization roles with {permission} permission " + ("-"*bar_size) + "+\n")
            ...
            terminal_width = shutil.get_terminal_size().columns
            bar_size = int((terminal_width - len(f"Documents with roles with {permission} permission") - 4) / 2)
            print("\n+" + ("-"*bar_size) + f" Documents with roles with {permission} permission " + ("-"*bar_size) + "+\n")
            ...
            for document_name in permission_roles["roles"]:
                print(f"Document {document_name}: {permission_roles["roles"][document_name]}")
