...
def rep_add_subject(state, session_file, username, name, email, credentials_file):
    ...
    if response.status_code == 200:
        ...
        print(f"Error when adding subject: {response.status_code}\nError Message: {response.json().get("Error")}")
        sys.exit(-1)
...
def rep_add_doc(state, session_file, document_name, file_path):
    ...
    encrypted_document_content, document_iv = encrypt_data_AES_CBC(file_content, secret_key)
    ...
    # Gerar o hmac
    message = session_id.encode() + encrypted_document_content + payload_bytes + payload_iv + nonce
    hmac_signature = calculate_hmac(message, integrity_key)

    final_payload = {
        "session_id": session_id,
        "encrypted_payload": base64.b64encode(encrypted_payload).decode(),
        "encrypted_document_content": base64.b64encode(encrypted_document_content).decode(),
...
def rep_get_doc_metadata(state, session_file, document_name=None):
    ...
    if response.status_code == 200:
        ...
        decrypted_payload = decrypt_data_AES_CBC(received_encrypted_payload, encryption_key, received_payload_iv)
