...
def createMinHashLookup():
    all_names = set(Standard_Unique_Names) | set(Departure_Unique_Names) | set(Transport_Unique_Names)
...
def createDepartureClusters():
    ...
    for departure_name in Departure_Unique_Names:
        count += 1
        if departure_name in seen_names:
            continue
        else:
            best_similarity = 0
            best_cluster = ""
    ...
    name_count = 0
    for center_name in clusters:
        name_count += len(clusters[center_name]) + 1
    ...
    with open(f"Departure_Clustering2.json", "w", encoding="utf-8") as f:
            json.dump(json_friendly_clusters, f, ensure_ascii=False, indent=4)


createDepartureClusters()

end_time = time.time()
elapsed_time = end_time - start_time
print(f"\nExecution time: {elapsed_time:.2f} seconds ({(elapsed_time/60.0):.2f} minutes)")