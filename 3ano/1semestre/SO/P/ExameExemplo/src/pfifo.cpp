...
void init_pfifo(PriorityFIFO* pfifo)
{
   ...
   mutex_init(&pfifo->mutex_access, NULL);
...
void term_pfifo(PriorityFIFO* pfifo)
{
   ...
   mutex_lock(&pfifo->mutex_access);
   while(!pfifo->is_closed)
   {
      cond_wait(&pfifo->closed_or_notEmpty, &pfifo->mutex_access);
   }
   mutex_unlock(&pfifo->mutex_access);
   require (is_closed_pfifo(pfifo), "FIFO open");

   mutex_destroy(&pfifo->mutex_access);
...
void insert_pfifo(PriorityFIFO* pfifo, int id, int priority)
{
   ...
   {
      cond_wait(&pfifo->closed_or_notFull, &pfifo->mutex_access);
   }
