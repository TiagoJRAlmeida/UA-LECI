...
void DateIncr(Date* d) {
  ...
  if(d->day < DateDaysInMonth(d->year, d->month)) d->day++;
  else if(d->month < 12){ d->month++; d->day = 1; }
  else{ d->year++; d->month = 1; d->day = 1; }
  assert(invariant(d));  // check invariant
}
...
void DateDecr(Date* d) {
  ...
  if(d->day > 1) d->day--;
  else if(d->month > 1){ d->month--; d->day = DateDaysInMonth(d->year, d->month); }
  else{ d->year--; d->month = 12; d->day = 31; }
  assert(invariant(d));  // check invariant
}
